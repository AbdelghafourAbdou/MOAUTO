...
if (isset($_POST['password']) && isset($_POST['email'])) {
    ...
    $query = " Select cusbalance from Customer where Cusemail = :email and cusPassword = :password";

     //prepare queries and bind values
    $stmt2 = $db_conn->prepare($query);
    ...
    if ($stmt2->execute()) {
        ...
        if ($user2) {
            $balance = $user2['cusbalance'];
            $_SESSION['cusb'] = $balance;
        }
    } else {
        echo $stmt->error;
    }
